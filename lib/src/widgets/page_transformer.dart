///
/// ORIGEM
/// https://github.com/FlutterRocks/page-transformer/blob/master/lib/page_transformer.dart
///
library page_transformer;
...
class PageVisibilityResolver {
  ...
  PageVisibility resolvePageVisibility(int pageIndex) {
    ...
    final visiblePageFraction = _calculateVisiblePageFraction(
      pageIndex,
      pagePosition,
    );
  ...
  double _calculatePagePosition(int index) {
    final viewPortFraction = this.viewPortFraction ?? 1.0;
    final pageViewWidth =
        (metrics?.viewportDimension ?? 1.0) * viewPortFraction;
    final pageX = pageViewWidth * index;
    final scrollX = metrics?.pixels ?? 0.0;
...
class _PageTransformerState extends State<PageTransformer> {
  ...
  Widget build(BuildContext context) {
    final pageView = widget.pageViewBuilder(
        context, _visibilityResolver ?? const PageVisibilityResolver());
